...
private:
    ...
    {
        using namespace std;
        cout << setw(4) << (uint32_t)params.addr << setw(4) << (uint32_t)params.cmd << setw(4) << (uint32_t)params.n;
    ...
    void PrintOperatingTime(uint8_t* data)
    {
        ...
        cout << "Operating Time: " << years << " years " << months << " months " << days << " days " << hours
             << " hours " << minutes << " minutes" << endl;
    }
public:
